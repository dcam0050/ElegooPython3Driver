...
class PyAudioSample:
    def __init__(self, source=None, num_samples=None, sample_rate=None, data=None, channels=None, transform=None, compression=None):
        self.source = source
    ...
    def encode_audio(self, data, sample_rate, channels):
        self.sample_rate = sample_rate
        self.channels = channels
        self.data = data.tostring()
        self.num_samples = int(len(data)/channels)
...
def decode_data(sample):
    ...
    elif sample.data_type == animus_data.DataMessage.MOTOR:
        return animus_data.MotorSample().FromString(sample.data)

    elif sample.data_type == animus_data.DataMessage.BLOB:
        return animus_data.BlobSample().FromString(sample.data)
...
def encode_data(sample):
    ...
    if isinstance(sample, list):
        if isinstance(sample[0], float):
            encoded_sample = animus_data.Float32Array(data=sample).SerializeToString()
            encoded_dtype = animus_data.DataMessage.FLOAT32ARR

        elif isinstance(sample[0], int):
            encoded_sample = animus_data.Int64Array(data=sample).SerializeToString()
    ...
    elif isinstance(sample, animus_data.MotorSample):
        encoded_sample = sample.SerializeToString()
        encoded_dtype = animus_data.DataMessage.MOTOR

    elif isinstance(sample, animus_data.BlobSample):
        encoded_sample = sample.SerializeToString()
        encoded_dtype = animus_data.DataMessage.BLOB
